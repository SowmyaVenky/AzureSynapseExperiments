...
public class MovieDataProcessor2 {
	private static final boolean WRITE_FILE_OUTPUTS = true;

	public static void main(String[] args) {
		if (args == null || args.length != 3) {
			System.out.println(
					"Need to pass 2 parameters - movies_metadata.csv, ratings.csv, and output folder for this to work!");
			System.exit(-1);
		}
		...
		Dataset<Row> moviesdf = spark.read().option("quote", "\"").option("escape", "\"").option("header", "true")
				.schema(moviesSchema).csv(moviesFile);

		Dataset<Row> movies_df = moviesdf.select(moviesdf.col("id").alias("movie_id"),
				moviesdf.col("adult").alias("is_adult"),
				moviesdf.col("budget"),
				moviesdf.col("homepage"),
				moviesdf.col("imdb_id"),
				moviesdf.col("video"),
				moviesdf.col("poster_path"),
		...
		if (WRITE_FILE_OUTPUTS) {
			...
			movies_df.repartition(1).write().mode(SaveMode.Overwrite).parquet(outputFolder + "/movies");
		}
		...
		if (WRITE_FILE_OUTPUTS) {
			...
			genrekeysremoved.repartition(1).saveAsTextFile(outputFolder + "/genre");
		}
		...
		if (WRITE_FILE_OUTPUTS) {
			...
			ratingsdf.repartition(1).write().mode(SaveMode.Overwrite).parquet(outputFolder + "/ratings");
		}
		...
		if (WRITE_FILE_OUTPUTS) {
			...
			movieGenreJson.repartition(1).saveAsTextFile(outputFolder + "/movie_genre");
		}
		spark.stop();
	}
