...
public class CreditsProcessor {
	private static final boolean WRITE_FILE_OUTPUTS = true;
	public static void main(String[] args) {
		
		if (args == null || args.length != 2) {
			System.out.println("Need to pass 2 parameters - credits.csv and output folder for this to work!");
			System.exit(-1);
		}
		...
		Dataset<Row> creditsdf = spark.read().option("quote", "\"").option("escape", "\"").option("header", "true")
				.csv(moviesFile);
		...
		Dataset<Row> castdf = creditsdf.filter("cast is not null")
		...
		Dataset<Row> castdf1 = castdf.select(castdf.col("movieId").as("movie_id").cast("int"),
		...
		if (WRITE_FILE_OUTPUTS) {
			...
			castdf1.repartition(1).write().mode(SaveMode.Overwrite).parquet(outputFolder + "/cast");
		}
		...
		Dataset<Row> crewdf = creditsdf.filter("crew is not null")
				.withColumn("crew_data",
						org.apache.spark.sql.functions.explode(org.apache.spark.sql.functions
								.from_json(creditsdf.col("crew"), DataTypes.createArrayType(crewSchema))))
				.drop("cast").drop("crew").withColumnRenamed("id", "movieId");
		
		Dataset<Row> crewdf1 = crewdf.select(
		...
		if (WRITE_FILE_OUTPUTS) {
			System.out.println("Writing crew file...");
			crewdf1.repartition(1).write().mode(SaveMode.Overwrite).parquet(outputFolder + "/crew");
		}
